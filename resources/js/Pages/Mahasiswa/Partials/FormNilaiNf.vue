...
const getNilaiKomponenByIdMahasiswa = () => {
    ...
    .then((res) => {
        if(res.data.data.nilai_kompeten_nf != null){
            nilai.nilai_kompeten_nf = JSON.parse(res.data.data.nilai_kompeten_nf)
        }
    })
    .catch((err) => {
        console.log(err)
...
const submit = () => {
    axios.put(`/updateNilai/${props.id}`, {
        nilai_kompeten: nilai.nilai_kompeten_nf,
        jenis_nilai: 'nilai_nf',
        jenis_bidang: props.jenis_bidang,
        jenis_nilai_kompeten: 'nilai_kompeten_nf',
    ...
    <form @submit.prevent="submit" class="space-y-6">
        <div v-for="(pertanyaan, index) in pertanyaans" class="flex">
            ...
            <InputLikertScale
                ...
                v-model="nilai.nilai_kompeten_nf[index]" />
        </div>
