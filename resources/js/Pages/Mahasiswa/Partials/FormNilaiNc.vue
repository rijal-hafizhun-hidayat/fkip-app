...
const getNilaiKomponenByIdMahasiswa = () => {
    ...
    .then((res) => {
        if(res.data.data.nilai_kompeten_nc != null){
            nilai.nilai_kompeten_nc = JSON.parse(res.data.data.nilai_kompeten_nc)
        }
    })
    .catch((err) => {
        console.log(err)
...
const submit = () => {
    axios.put(`/updateNilai/${props.id}`, {
        nilai_kompeten: nilai.nilai_kompeten_nc,
        jenis_nilai: 'nilai_nc',
        jenis_bidang: props.jenis_bidang,
        jenis_nilai_kompeten: 'nilai_kompeten_nc',
    ...
    <form @submit.prevent="submit" class="space-y-6">
        <div v-for="(pertanyaan, index) in pertanyaans" class="flex">
            ...
            <InputLikertScale
                ...
                v-model="nilai.nilai_kompeten_nc[index]" />
        </div>
