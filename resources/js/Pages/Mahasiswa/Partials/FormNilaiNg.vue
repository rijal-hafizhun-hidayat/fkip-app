...
const getNilaiKomponenByIdMahasiswa = () => {
    ...
    .then((res) => {
        if(res.data.data.nilai_kompeten_ng != null){
            nilai.nilai_kompeten_ng = JSON.parse(res.data.data.nilai_kompeten_ng)
        }
    })
    .catch((err) => {
        console.log(err)
...
const submit = () => {
    ...
    axios.put(`/updateNilai/${props.id}`, {
        nilai_kompeten: nilai.nilai_kompeten_ng,
        jenis_nilai: 'nilai_ng',
        jenis_bidang: props.jenis_bidang,
        jenis_nilai_kompeten: 'nilai_kompeten_ng',
    ...
    <form @submit.prevent="submit" class="space-y-6">
        <div v-for="(pertanyaan, index) in pertanyaans" class="flex">
            ...
            <InputLikertScale
                ...
                v-model="nilai.nilai_kompeten_ng[index]" />
        </div>
