...
const getNilaiKomponenByIdMahasiswa = () => {
    ...
    .then((res) => {
        if(res.data.data.nilai_kompeten_na != null){
            nilai.nilai_kompeten_na = JSON.parse(res.data.data.nilai_kompeten_na)
        }
    })
    .catch((err) => {
        console.log(err)
...
const submit = () => {
    axios.put(`/updateNilai/${props.id}`, {
        nilai_kompeten: nilai.nilai_kompeten_na,
        jenis_nilai: 'nilai_na',
        jenis_bidang: props.jenis_bidang,
        jenis_nilai_kompeten: 'nilai_kompeten_na',
    ...
    <form @submit.prevent="submit" class="space-y-6">
        <div v-for="(pertanyaan, index) in pertanyaans" class="flex">
            ...
            <InputLikertScale
                ...
                v-model="nilai.nilai_kompeten_na[index]" />
        </div>
